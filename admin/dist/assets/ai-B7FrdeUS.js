import{bn as d,bg as g,au as m,ae as p,av as h}from"./index-DUVSbVOL.js";import{c as $}from"./utils-6XjqX0MW.js";function y(){let e,r,a,o=!1;const n=[];return{async start(){if(o)return;o=!0;const s=new Blob([`
...
`+n,"error"),console.error("useAi::transcribe(): Error transcribing from media URL",n)})}function U(e,r,a=null,o=null){const n=d(),s=g(),{$gettext:t}=m;if(!n.can("text:translate")){s.add(t("Permission denied"),"error");return}return Array.isArray(e)||(e=[e].filter(i=>!!i)),e.length?r?p.mutate({mutation:h`
...
`+i,"error"),console.error("useAi::translate(): Error translating texts",i)}):Promise.reject(new Error("Target language is required")):Promise.resolve([])}function M(e,r=[],a=[]){const o=d(),n=g(),{$gettext:s}=m;if(!o.can("text:write")){n.add(s("Permission denied"),"error");return}return e=String(e).trim(),e?(Array.isArray(r)||(r=[r]),r.push("Only return the requested data without any additional information"),p.mutate({mutation:h`
...
`+t,"error"),console.error("useAi::write(): Error generating text",t)})):(n.add(s("Prompt is required for generating text"),"error"),Promise.resolve(""))}export{U as a,y as r,B as t,M as w};
