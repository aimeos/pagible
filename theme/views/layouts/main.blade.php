...
<html class="no-js" data-theme="light" lang="{{ cms($page, 'lang') }}" dir="{{ in_array(cms($page, 'lang'), ['ar', 'az', 'dv', 'fa', 'he', 'ku', 'ur']) ? 'rtl' : 'ltr' }}">
    <head>
        ...
        {{-- Emit raw and byte-tight: the inner content must match cmshashes('config.styles.data.text') --}}
        @foreach($page->ancestorsAndSelf as $navItem)
            @if($text = cms($navItem, 'config.styles.data.text'))<style>{!! $text !!}</style>@endif
        @endforeach
    ...
    <body class="theme-clean type-{{ cms($page, 'type', 'page') }}">
        ...
        {{-- Emit raw and byte-tight: the inner content must match cmshashes('config.javascript.data.text') --}}
        @foreach($page->ancestorsAndSelf as $navItem)
            @if($text = cms($navItem, 'config.javascript.data.text'))<script>{!! $text !!}</script>@endif
        @endforeach
